var PORT = 0x88AC;
//BH ORG: var HOST = '192.168.0.59';
var HOST = '192.168.0.18';
var ULSWITCH = '192.168.0.3';

var mqtt = require('mqtt')
var mclient = mqtt.connect({ port: 1883, host: '192.168.0.70', protocol: 'mqtt' });
//BH Org: var mclient = mqtt.connect({ port: 1883, host: '192.168.0.70', protocol: 'mqtt' });
...
function SendDatagram(datagram) {
  //server.send(datagram, 0, datagram.length, PORT, '192.168.0.101', function(err, bytes) {
    server.send(datagram, 0, datagram.length, PORT, ULSWITCH, function(err, bytes) {  
    if (err) throw err;
  })
};
...
function aap(welkeaap)
{
  console.log("aap: " + welkeaap);
...
//BH Commented below. was in org
//setTimeout(GoToPage(5),10000);
setTimeout(function(){GoToPage(5);},5000);

//BH werkt: setTimeout(function(){aap(1);},5000);
